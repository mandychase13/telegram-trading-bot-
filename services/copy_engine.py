...
from blockchain.solana_executor import (
    execute_sol_buy,
    execute_sol_sell,
    get_sol_signer_address,
...
async def _execute_copy_trade(
    ...
    if chain == "SOL":
        try:
            execution_address = get_sol_signer_address(pk)
        except Exception as exc:
            logger.error("Could not derive Solana signing address for user_id=%s: %s", user_id, exc)
            return
        stored_address = wallet.get("sol_address") or ""
        if stored_address and stored_address != execution_address:
            logger.error(
                "Wallet signer mismatch user_id=%s stored_address=%s signer_address=%s",
                user_id, stored_address, execution_address,
            )
            return

    # Only verified on-chain funds can authorize a live BUY. An admin credit is
    # an accounting-only value and does not fund the wallet that signs a swap.
    onchain_balance = await _get_native_balance(
        chain, user_id, address=execution_address
    )
    source_native_amount = Decimal(str(trade_info.get("amount_in") or 0))
    if source_native_amount <= 0:
        return
    requested_native_amount = source_native_amount * copy_pct
    native_trade_amount = min(requested_native_amount, max_amount)
    if native_trade_amount <= 0 or native_trade_amount < min_amount:
        return

    if trade_type == "buy":
        available_onchain = Decimal(str(await resolve_available_balance(
            user_id, chain, chain, onchain_balance
        )))
        if chain == "SOL":
            fee_reserve = Decimal(SOL_FEE_RESERVE_LAMPORTS) / Decimal("1000000000")
            available_onchain = max(Decimal("0"), available_onchain - fee_reserve)
        copy_amount = min(native_trade_amount, available_onchain)
    else:
        # The source amount_in is the native asset spent/received. The executor
        # needs the token quantity, so preserve the source trade's ratio while
        # applying the existing native-asset max_trade_amount setting.
        source_token_amount = Decimal(str(trade_info.get("token_amount") or 0))
        if source_token_amount <= 0:
            logger.warning(
                "Copy SELL has no token quantity user_id=%s source_tx=%s",
                user_id, trade_info.get("tx_hash", ""),
            )
            return
        copy_ratio = native_trade_amount / source_native_amount
        copy_amount = source_token_amount * copy_ratio

    copy_amount = copy_amount.quantize(Decimal("0.000000001"), rounding=ROUND_DOWN)
    if copy_amount <= 0 or (trade_type == "buy" and copy_amount < min_amount):
        return
    executed_native_amount = copy_amount if trade_type == "buy" else native_trade_amount
    ...
    if not source_tx_hash:
        logger.warning(
            "Skipping copy event without a source transaction id user_id=%s followed_wallet_id=%s",
            user_id, followed_id,
        )
        return
    ...
    if result["ok"]:
        ...
        await apply_copy_position_update(
            ...
            trade_type=trade_type,
            amount=float(
                Decimal(str(trade_info.get("token_amount") or 0))
                * (executed_native_amount / source_native_amount)
            ),
        )
        status_icon = "✅"
...
async def _get_native_balance(
    chain: str, user_id: int, address: Optional[str] = None
) -> float:
    ...
    try:
        if chain == "SOL":
            return await get_sol_balance(address or wallet.get("sol_address") or "")
        if chain == "ETH":
