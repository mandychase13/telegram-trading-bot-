...
async def _execute_copy_trade(
    ...
    copy_pct      = (cs.get("copy_percentage", 10.0)   if cs else 10.0) / 100.0
    max_amount    = cs.get("max_trade_amount",  1.0)    if cs else 1.0
    slippage_bps  = int((cs.get("slippage",    1.0)     if cs else 1.0) * 100)

    copy_amount = min(trade_info["amount_in"] * copy_pct, max_amount)
    if copy_amount <= 0:
        return
