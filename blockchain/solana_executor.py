...
async def _jupiter_swap(
    ...
    try:
        ...
        logger.error("Jupiter swap network failure quote_host=%s swap_host=%s: %s",
                     settings.endpoint_host(JUPITER_QUOTE_URL),
                     settings.endpoint_host(JUPITER_SWAP_URL), exc)
        return {"ok": False, "error": f"Jupiter network failure: {exc}"}
    except Exception as exc:
        logger.error("Jupiter swap failed quote_host=%s swap_host=%s: %s",
                     settings.endpoint_host(JUPITER_QUOTE_URL),
                     settings.endpoint_host(JUPITER_SWAP_URL), exc)
