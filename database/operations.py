...
async def get_internal_balances(user_id: int) -> dict[str, dict]:
    rows = await fetchall(
        """
        SELECT asset, network, COALESCE(admin_credit, balance) AS balance,
...
async def save_copy_trade_once(
    user_id: int,
    chain: str,
    trade_type: str,
    token_address: str,
    token_symbol: str,
    amount_in,
    followed_wallet_id: int,
    source_tx_hash: str,
    status: str = "pending",
) -> Optional[dict]:
    """Insert one copy trade per followed-wallet source event.

    PostgreSQL owns the race protection through the unique partial index, so
    two overlapping polling cycles cannot execute the same trader event twice.
    """
    return await fetchone_returning(
        """
        INSERT INTO trades
            (user_id, chain, trade_type, token_address, token_symbol, amount_in,
             is_copy_trade, followed_wallet_id, status, source_tx_hash)
        VALUES (%s, %s, %s, %s, %s, %s, TRUE, %s, %s, %s)
        ON CONFLICT (user_id, followed_wallet_id, source_tx_hash)
        WHERE is_copy_trade = TRUE AND source_tx_hash IS NOT NULL AND source_tx_hash <> ''
        DO UPDATE SET status = 'pending'
        WHERE trades.status = 'failed'
        RETURNING *
        """,
        (
            user_id, chain, trade_type, token_address, token_symbol, amount_in,
            followed_wallet_id, status, source_tx_hash,
        ),
    )


async def copy_trade_already_recorded(
    user_id: int, followed_wallet_id: int, source_tx_hash: str
) -> bool:
    ...
    row = await fetchone(
        ...
        WHERE user_id = %s AND followed_wallet_id = %s
          AND is_copy_trade = TRUE AND source_tx_hash = %s
        LIMIT 1
        """,
        (user_id, followed_wallet_id, source_tx_hash),
    )
...
async def apply_copy_position_update(
    user_id: int,
    chain: str,
    token_address: str,
    token_symbol: str,
    ...
    """Keep the copy position open/closed without replacing another fill.

    The executor currently reports the requested trade amount, so this is a
    conservative internal position quantity update. It never changes wallet
    balances or creates a blockchain transaction.
    """
    if trade_type == "buy":
        await execute(
            """
            INSERT INTO portfolio_tokens
                (user_id, chain, token_address, token_symbol, balance)
            VALUES (%s, %s, %s, %s, %s)
            ON CONFLICT (user_id, chain, token_address)
            DO UPDATE SET
                balance = portfolio_tokens.balance + EXCLUDED.balance,
                updated_at = NOW()
            """,
            (user_id, chain, token_address, token_symbol, amount),
        )
        return
    ...
    await execute(
        ...
        SET balance = GREATEST(0, balance - %s), updated_at = NOW()
        WHERE user_id = %s AND chain = %s AND token_address = %s
        """,
        (amount, user_id, chain, token_address),
    )
