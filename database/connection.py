...
def _apply_balance_adjustment_sync(
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            previous = Decimal(str(row[1] or row[0]))
            ...
            cur.execute(
                ...
                SET balance = %s, admin_credit = %s,
                    display_mode = 'internal', updated_at = NOW()
                WHERE user_id = %s AND asset = %s AND network = %s
                """,
                (new_balance, new_balance, user_id, asset, network),
            )
