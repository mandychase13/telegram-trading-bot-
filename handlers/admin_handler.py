...
async def admin_balance_user_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not user:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="User not found.", reply_markup=InlineKeyboardMarkup([[
    ...
    if not wallet:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="This user has no wallet.", reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("⬅️ Back", callback_data="admin:balance")
        ]]))
        return AT_BAL_SELECT_USER
...
async def admin_balance_chain_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text=
        ...
        f"Copy-trading allocation: `{fmt_balance(float(onchain + current))} {chain}`\n"
...
async def admin_user_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="⏳ Fetching live balances…")
    try:
