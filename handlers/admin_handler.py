...
async def admin_balance_user_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not user:
        await context.bot.send_message(update.effective_chat.id, "User not found.", reply_markup=InlineKeyboardMarkup([[
    ...
    if not wallet:
        await context.bot.send_message(update.effective_chat.id, "This user has no wallet.", reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("⬅️ Back", callback_data="admin:balance")
        ]]))
        return AT_BAL_SELECT_USER
...
async def admin_balance_chain_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(update.effective_chat.id,
        ...
        f"Admin trading credit: `{fmt_balance(float(current))} {chain}`\n"
         f"Live execution funds: `{fmt_balance(float(onchain))} {chain}`\n"
         f"Source: _{mode}_\n"
         "Admin credit is accounting-only and cannot pay blockchain fees or swaps.\n\n"
...
async def admin_user_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(update.effective_chat.id, "⏳ Fetching live balances…")
    try:
