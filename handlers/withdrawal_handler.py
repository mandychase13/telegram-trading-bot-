...
async def withdrawal_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not wallet:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ No wallet found. Type /start.")
        return ConversationHandler.END
...
async def admin_approve_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not wd:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Withdrawal record not found.")
        return
    ...
    if not wallet:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"❌ Withdrawal #{wd_id} failed: wallet not found.")
        return
