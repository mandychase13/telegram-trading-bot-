...
async def admin_approve_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not wd:
        await context.bot.send_message(update.effective_chat.id, "❌ Withdrawal record not found.")
        return
    ...
    if not wallet:
        ...
        await context.bot.send_message(update.effective_chat.id, f"❌ Withdrawal #{wd_id} failed: wallet not found.")
        return
