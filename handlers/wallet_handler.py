...
async def _build_wallet_text(wallet: dict) -> tuple[str, object]:
    ...
    sol_credit, eth_credit, bnb_credit = await asyncio.gather(
        get_admin_credit(wallet["user_id"], "SOL", "SOL"),
        get_admin_credit(wallet["user_id"], "ETH", "ETH"),
        get_admin_credit(wallet["user_id"], "BNB", "BNB"),
    )
    ...
    text = (
        ...
        f"  On-chain balance: {fmt_balance(sol_bal)} SOL ≈ {fmt_usd(sol_bal * prices.get('SOL', 0))}\n\n"
        f"Ξ *Ethereum*\n"
        f"  Address: `{wallet['eth_address'] or ''}`\n"
        f"  On-chain balance: {fmt_balance(eth_bal)} ETH ≈ {fmt_usd(eth_bal * prices.get('ETH', 0))}\n\n"
        f"🟡 *BNB Chain*\n"
        f"  Address: `{wallet['bnb_address'] or ''}`\n"
        f"  On-chain balance: {fmt_balance(bnb_bal)} BNB ≈ {fmt_usd(bnb_bal * prices.get('BNB', 0))}\n\n"
        f"📒 *Admin accounting credit:* {fmt_balance(sol_credit)} SOL · "
        f"{fmt_balance(eth_credit)} ETH · {fmt_balance(bnb_credit)} BNB "
        f"_(not spendable on-chain)_\n\n"
...
async def wallet_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    wallet = await get_wallet(db_user["id"])
    if not wallet:
        await context.bot.send_message(update.effective_chat.id, "❌ No wallet found. Type /start.")
        return
    ...
    try:
        ...
        await context.bot.send_message(update.effective_chat.id, "❌ Could not load wallet. Please try again.")
