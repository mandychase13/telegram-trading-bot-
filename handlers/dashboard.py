...
async def _build_dashboard_text(tg_user_id: int) -> str:
    ...
    sol_bal, eth_bal, bnb_bal = await asyncio.gather(
        resolve_available_balance(user_id, "SOL", "SOL", sol_onchain),
        resolve_available_balance(user_id, "ETH", "ETH", eth_onchain),
        resolve_available_balance(user_id, "BNB", "BNB", bnb_onchain),
    )
    portfolio_usd = (
    ...
    text = (
        ...
        f"💵 *SOL Balance:* {fmt_balance(sol_bal)} SOL\n"
        f"💵 *ETH Balance:* {fmt_balance(eth_bal)} ETH\n"
        f"💵 *BNB Balance:* {fmt_balance(bnb_bal)} BNB\n\n"
