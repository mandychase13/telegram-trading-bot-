...
async def buy_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not db_user:
        await query.edit_message_text("❌ Account error. Type /start.")
        return ConversationHandler.END

    wallet = await get_wallet(db_user["id"])
    if not wallet:
        await query.edit_message_text("❌ Wallet not found. Type /start.")
        return ConversationHandler.END
...
async def sell_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("⏳ *Executing sell…*", parse_mode="Markdown")

    tg_user = update.effective_user
    db_user = await get_user(tg_user.id)
    if not db_user:
        await query.edit_message_text("❌ Account error.")
        return ConversationHandler.END
...
async def tf_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("⏳ *Sending transfer…*", parse_mode="Markdown")
