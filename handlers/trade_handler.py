...
async def buy_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await context.bot.send_message(chat_id=update.effective_chat.id, text="⏳ *Executing buy…*", parse_mode="Markdown")

    tg_user = update.effective_user
    db_user = await get_user(tg_user.id)
    if not db_user:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Account error. Type /start.")
        return ConversationHandler.END

    wallet = await get_wallet(db_user["id"])
    if not wallet:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Wallet not found. Type /start.")
        return ConversationHandler.END
...
async def sell_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await context.bot.send_message(chat_id=update.effective_chat.id, text="⏳ *Executing sell…*", parse_mode="Markdown")
    ...
    wallet = await get_wallet(db_user["id"])
    if not wallet:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Wallet not found.")
        return ConversationHandler.END
...
async def tf_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await context.bot.send_message(chat_id=update.effective_chat.id, text="⏳ *Sending transfer…*", parse_mode="Markdown")
