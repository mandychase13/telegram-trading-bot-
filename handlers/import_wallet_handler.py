...
async def import_wallet_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not db_user:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Account not found. Type /start.")
...
async def import_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not wallet:
        _cleanup(context)
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ No wallet record. Type /start first.")
        return ConversationHandler.END
    ...
    if not (chain and address and pk_hex):
        _cleanup(context)
        await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ Session data lost. Please start over.")
        return ConversationHandler.END
...
async def _finish_pending_withdrawal_import(
    ...
    """Send a fresh withdrawal status message after a successful wallet import."""
    ...
    # If the process restarted between the withdrawal and import, still show
